...
def show_tfidf(tfidf, vocab, filename):
    ...
    plt.yticks(np.arange(tfidf.shape[0]), np.arange(
        1, tfidf.shape[0]+1), fontsize=6)
    plt.tight_layout()
...
def show_w2v_word_embedding(model, data: utils.Dataset, path):
    ...
    for i in range(data.num_word):
        ...
        try:
            ...
            c = "red"
        plt.text(word_emb[i, 0], word_emb[i, 1],
                 s=data.i2v[i], color=c, weight="bold")
...
def seq2seq_attention():
    ...
    for i in range(6):
        ...
        if i >= 3:
            plt.xlabel("Input")
    plt.tight_layout()
    plt.savefig("./visual/results/seq2seq_attention.png",
                format="png", dpi=200)
    plt.show()


def all_mask_kinds():
    seqs = ["I love you", "My name is M",
            "This is a very long seq", "Short one"]
    ...
    # padding mask
    pmask = np.where(padded_id_seqs == 0, np.ones_like(
        padded_id_seqs), np.zeros_like(padded_id_seqs))  # 0 idx is padding
    pmask = np.repeat(pmask[:, None, :], pmask.shape[-1],
                      axis=1)  # [n, step, step]
    plt.rcParams['xtick.bottom'] = plt.rcParams['xtick.labelbottom'] = False
    plt.rcParams['xtick.top'] = plt.rcParams['xtick.labeltop'] = True
    for i in range(1, 5):
    ...
    omask = np.tile(np.expand_dims(omask, axis=0), [
                    np.shape(seqs)[0], 1, 1])  # [n, step, step]
...
def position_embedding():
    ...
    pe = pos / np.power(10000, 2. * np.arange(model_dim)
                        [None, :] / model_dim)  # [max_len, model_dim]
...
def transformer_attention_matrix(case=0):
    ...
    for i in range(3):
        for j in range(4):
            ...
            plt.imshow(encoder_atten[i][case, j][:len(
                src), :len(src)], vmax=1, vmin=0, cmap="rainbow")
    ...
    plt.savefig(
        "./visual/results/transformer%d_encoder_self_attention.png" % case, dpi=200)
    plt.show()
    ...
    for i in range(3):
        for j in range(4):
            plt.subplot(3, 4, i * 4 + j + 1)
            plt.imshow(decoder_tgt_atten[i][case, j][:len(
                tgt), :len(tgt)], vmax=1, vmin=0, cmap="rainbow")
    ...
    plt.subplots_adjust(top=0.9)
    plt.savefig(
        "./visual/results/transformer%d_decoder_self_attention.png" % case, dpi=200)
    plt.show()
    ...
    for i in range(3):
        for j in range(4):
            plt.subplot(3, 4, i*4+j+1)
            plt.imshow(decoder_src_atten[i][case, j][:len(
                tgt), :len(src)], vmax=1, vmin=0, cmap="rainbow")
    ...
    plt.subplots_adjust(top=0.9)
    plt.savefig(
        "./visual/results/transformer%d_decoder_encoder_attention.png" % case, dpi=200)
    plt.show()
...
def transformer_attention_line(case=0):
    ...
    for i in range(2):
        for j in range(2):
            ...
            ax_ = ax[i, j].twinx()
            ax_.set_yticks(np.linspace(ax_.get_yticks()[
                           0], ax_.get_yticks()[-1], len(ax[i, j].get_yticks())))
            ...
            for ri, c in zip(range(right_top), color):      # tgt
                for li in range(left_top):                 # src
                    ...
                    ax[i, j].plot([0, 1], [left_top - li + 1,
                                  right_top - 1 - ri], alpha=alpha, c=c)
            ax[i, j].set_xticks(())
    ...
    plt.tight_layout()
    plt.savefig(
        "./visual/results/transformer%d_encoder_decoder_attention_line.png" % case, dpi=100)
...
def self_attention_matrix(bert_or_gpt="bert", case=0):
    ...
    for j in range(4):
        ...
        plt.xticks(range(s_len-1), src[case]
                   [:s_len-1], rotation=90, fontsize=9)
        plt.yticks(range(s_len-1), src[case][1:s_len], fontsize=9)
        plt.xlabel("head %i" % (j+1))
    plt.subplots_adjust(top=0.9)
    plt.tight_layout()
    plt.savefig("./visual/results/"+bert_or_gpt +
                "%d_self_attention.png" % case, dpi=500)
    # plt.show()
...
def self_attention_line(bert_or_gpt="bert", case=0):
        ...
        for j in range(2):
            ...
            for row, c in zip(range(img.shape[0]), color):
                for col in range(img.shape[1]):
                    ...
                    ax[i, j].plot([0, 1], [img.shape[1]-col,
                                  img.shape[0]-row-1], alpha=alpha, c=c)
            ax[i, j].set_xticks(())
            ax[i, j].set_xlabel("head %i" % (j+1+i*2))
            ax[i, j].set_xlim(0, 1)
    plt.subplots_adjust(top=0.9)
    plt.tight_layout()
    plt.savefig("./visual/results/"+bert_or_gpt +
                "%d_self_attention_line.png" % case, dpi=100)
