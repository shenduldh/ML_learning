...
class DateData:
    def __init__(self, n):
        ...
        self.v2i = {v: i for i, v in enumerate(
            sorted(list(self.vocab)), start=1)}
        self.v2i["<PAD>"] = PAD_ID
        self.vocab.add("<PAD>")
...
def _process_mrpc(dir="./MRPC", rows=None):
    ...
    for f in files:
        ...
        data[k] = {"is_same": df.iloc[:, 0].values,
                   "s1": df["#1 String"].values, "s2": df["#2 String"].values}
    vocab = set()
    for n in ["train", "test"]:
        for m in ["s1", "s2"]:
    ...
    for n in ["train", "test"]:
        for m in ["s1", "s2"]:
            data[n][m+"id"] = [[v2i[v]
                                for v in c.split(" ")] for c in data[n][m]]
    return data, v2i, i2v
    ...
    def __init__(self, data_dir="./MRPC/", rows=None, proxy=None):
        ...
        x = [
            [self.v2i["<GO>"]] + data["train"]["s1id"][i] + [self.v2i["<SEP>"]
                                                             ] + data["train"]["s2id"][i] + [self.v2i["<SEP>"]]
...
class MRPCSingle:
    ...
    def __init__(self, data_dir="./MRPC/", rows=None, proxy=None):
        ...
        self.max_len = max([len(s) + 2 for s in data["train"]
                           ["s1id"] + data["train"]["s2id"]])
        x = [
            [self.v2i["<GO>"]] + data["train"]["s1id"][i] + [self.v2i["<SEP>"]]
        ...
        self.word_ids = np.array(
            list(set(self.i2v.keys()).difference([self.v2i["<PAD>"]])))
